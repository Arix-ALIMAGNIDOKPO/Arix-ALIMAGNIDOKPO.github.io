...
| Colours, type scale, spacing, both themes          | `src/styles/global.css`     |
...
shipped drives the article carousel and the theme toggle; with JS disabled the
carousel is still a scrollable list, the theme follows the system preference,
and every page is fully readable.

### Themes

The site ships a light and a dark theme. With no stored choice it follows the
viewer's system preference; the toggle in the header sets `data-theme` on
`<html>` and stores the choice in `localStorage`. A small blocking script in
`<head>` applies it before the first paint so there is no flash of the wrong
theme. Both palettes are declared as custom properties at the top of
`global.css`: change a colour once and it changes everywhere.

### House style

No em dashes anywhere in the site copy. Date ranges use an en dash
(`Aug 2024 - Apr 2025`) and ongoing roles read `Since May 2025`, which is
generated by `formatPeriod` in `src/lib/dates.ts`.
